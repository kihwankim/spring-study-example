...
class CustomJobListener : JobListener {
    ...
    override fun jobToBeExecuted(context: JobExecutionContext) {
        log.info("Job is about to be executed: " + context.jobDetail.key)
    }

    // Job 실행이 중단되었을 때 호출
    override fun jobExecutionVetoed(context: JobExecutionContext) {
        log.info("Job execution was vetoed: " + context.jobDetail.key)
    }
    ...
    override fun jobWasExecuted(
        ...
        if (jobException != null) {
            log.info("Exception thrown by job: " + jobException.message)
        }
    }
}