...
- 저 또한 트랜젝션을 어떻게 처리할지 부터, 에러 발생시 어떻게 rollback을 해야할지 감을 못잡았습니다.
- rollback을 처리 하기 위해서 try catch 가 난무한 코드가 되기도 하였고, 이걸 해결하기 위해서 handler를 람다로 호출 하도록 해보고, retry 가능하게 멱등성 보존 API로 변경한 작도 있었습니다.
- 최종적으로 Retry 가능 영역과, 실패시 rollback을 처리 부분, 백오피스에서 데이터 클랜징 작업을 해야 하는 영역을 분리하고 로직을 작성하였습니다.
- 이렇게 저의 경험과 회고를 남기는 이유는 많은 책을 읽고 Hex 아키텍처에 대해서 저의 의견을 작성하였지만, 저의 주간이 어쩔 수 없게 들어 갔습니다. 결론적으로 언제나 틀린 글을 작성할 수 있기 때문에 동의 하지 않는 부분이 있을 경우 그 부분은 무시해주시면 됩니다.
- 그리고 추가적으로 더 좋은 의견이 있다면, 댓글이나 Git Issue로 등록해 주시면 좋을거 같습니다.